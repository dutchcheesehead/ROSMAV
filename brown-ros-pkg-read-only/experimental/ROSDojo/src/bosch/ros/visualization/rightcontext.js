...
0.2.5 : 04 Oct 2008  Fixes tags not passing properly on FF3 with onclick events.
 ...
 Last Change : Jihoon Lee  08.2011
 It only hooks canvas's right click
...
var RightContext = {
    ...
    var canvas = document.getElementById(canvas_id);
		this.attachContextEvents(canvas);   
...
//		return this.menuCollection[n];
	...
	attachContextEvents: function (canvas) {
		var tagContext, thisTag;

		thisTag = canvas;
    
		this.bindEvent('mousemove', canvas, function(e) { return RightContext.locateMousePos(e); });
  	if (this.menuTriggerEvent=="RIGHT") {
	    canvas.oncontextmenu = function() {   return RightContext.render(this);   }; 
		} 
	},
...
//		this.repositionMenu();
    ...
    if (item["command_type"] != null && item["command_type"] != "undefined")
    {
      cell.setAttribute("command_type",item["command_type"]);
    }

    if (item["command"] != null && item["command"] != "undefined")
    {
      if(item["command"] == "")
        cell.setAttribute("command","");
      else 
        cell.setAttribute("command",item["command"]);
    }
	...
	buildMenu: function (parent) {
		...
		this.position (this.contextMenu, this.mousePos.x + this.rightOffset, this.mousePos.y - pos.y);
		...
		this.parentdiv.appendChild(this.contextMenu);
